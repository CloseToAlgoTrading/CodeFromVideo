...
class MyTradeEnv(py_environment.PyEnvironment):
    ...
    def __get_observation_data(self, idx):
        _x = self._selectedData.copy()
        _x[idx:] = 0
        #_x = MinMaxScaler().fit_transform(self._selectedData).astype(np.float32)
        _x = MinMaxScaler().fit_transform(_x).astype(np.float32)
        #print(_x)
        return _x
    ...
    def __select_episode(self, idx):
        ...
        if(self.change_stock_counter > self.change_stock_step):
            ...
            _, self.price_data = self.__getRandomDataFrom(self.stocks, self.data_range, verbose=1)
    ...
    def __init__(self, stocks, data_range, isMOC = True):
        ...
        #define price data
        _, self.price_data = self.__getRandomDataFrom(self.stocks, self.data_range)
    ...
    def __open_position(self, pos, action, price_index):
        ...
        if(self._position == 0): #no open positions
            ...
            if(needMoney > self._money): #no more money to trade => exit
                ...
                actionPnl = self.__calculatePNL(pos_state='open', price_index=price_index) # self._fees - self._spread*self._shares # calculate immidiade pnl
    ...
    def __calculatePNL(self, pos_state, price_index, isPercentage = True):
        ...
        #print(self._fees, self._spread, self._shares, self._priceArray[price_index])
        if pos_state == 'open':
            if isPercentage == True:
                #ret = ((self._fees - self._spread*self._shares) * 100)/ self._priceArray[price_index]
                ret = ((self._fees - self._spread) * 100) / self._priceArray[price_index]
            else:
            ...
            if isPercentage == True:
                #ret = ((diffPrice * self._shares) * 100) / self._priceArray[1]
                ret = ((diffPrice) * 100) / self._priceArray[1]
            else:
                ret = diffPrice * self._shares


        elif pos_state == 'close':
            diffPrice = self.__calculate_price_change(1, 0)
            if isPercentage == True:
                #ret = ((diffPrice * self._shares + self._fees - self._spread*self._shares) * 100) / self._priceArray[1]
                ret = ((diffPrice + self._fees - self._spread) * 100) / self._priceArray[1]
            else:
                ret = diffPrice * self._shares + self._fees - self._spread*self._shares
        #print('<---------------- ret', ret )
        return ret

    #def __calculateImmidiatePNL(self, price_index, isPercentage = True):
    #    if isPercentage:
    #        return ((self._fees - self._spread*self._shares) * 100)/ self._priceArray[price_index]
    #    else:
    #        return self._fees - self._spread*self._shares

    def __calculate_price_change(self, i1, i2):
        return (np.abs(self._priceArray[i1]) - np.abs(self._priceArray[i2]))*self._position

    #def __calculate_close_position_pnl(self, diffPrice, i1 = 1, isPercentage = True):
    #    if isPercentage:
    #        return ((diffPrice * self._shares + self._fees - self._spread*self._shares) * 100) / self._priceArray[i1]
    #    else:
