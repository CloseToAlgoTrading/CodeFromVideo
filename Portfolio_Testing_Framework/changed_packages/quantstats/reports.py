...
def html2(m_returns, benchmark=None, rf=0., grayscale=False,
         title='Strategy Tearsheet', output=None, compounded=True,
         periods_per_year=252, download_filename='quantstats-tearsheet.html',
         figfmt='svg', template_path=None, match_dates=False, **kwargs):
    ...
    # prepare timeseries
    m_returns = _utils._prepare_returns(m_returns)
    ...
    if benchmark is not None:
        ...
        tpl = tpl.replace('{{benchmark_title}}', f"Benchmark is {benchmark_title.upper()} | ")
        benchmark = _utils._prepare_benchmark(benchmark, m_returns.index, rf)
        if match_dates is True:
            m_returns, benchmark = _match_dates(m_returns, benchmark)

    date_range = m_returns.index.strftime('%e %b, %Y')
    tpl = tpl.replace('{{date_range}}', date_range[0] + ' - ' + date_range[-1])
    tpl = tpl.replace('{{title}}', title)
    tpl = tpl.replace('{{v}}', __version__)

    mtrx = _pd.DataFrame()
    for i,c in enumerate(m_returns.columns.values):
       # if(i == n_col-1):
        _mtrx = metrics(returns=m_returns.loc[:,c], benchmark=benchmark,
                       rf=rf, display=False, mode='Basic',
                       sep=True, internal="True",
                       compounded=compounded,
                       periods_per_year=periods_per_year,
                       prepare_returns=False)[2:]
        _mtrx.rename(columns = {'Strategy':c}, inplace=True)
        if(i == 0):
            mtrx = _pd.concat([mtrx,_mtrx.loc[:,['Benchmark',c]].copy()], axis=1)
        else:
            mtrx = _pd.concat([mtrx,_mtrx.loc[:,c].copy()], axis=1)
    ...
    # dd_info.columns = ['Started', 'Recovered', 'Drawdown', 'Days']
    # tpl = tpl.replace('{{dd_info}}', _html_table(dd_info, False))
    tpl = tpl.replace('{{dd_title}}', '')
    # plots
    figfile = _utils._file_stream()
    _plots.returns(m_returns, benchmark, grayscale=grayscale,
                   figsize=(8, 5), subtitle=False,
                   savefig={'fname': figfile, 'format': figfmt},
                   show=False, ylabel=False, cumulative=compounded,
                   prepare_returns=False)
    ...
    figfile = _utils._file_stream()
    _plots.log_returns(m_returns, benchmark, grayscale=grayscale,
                       figsize=(8, 4), subtitle=False,
                       savefig={'fname': figfile, 'format': figfmt},
                       show=False, ylabel=False, cumulative=compounded,
                       prepare_returns=False)
    ...
    if benchmark is not None:
        figfile = _utils._file_stream()
        _plots.returns(m_returns, benchmark, match_volatility=True,
                       grayscale=grayscale, figsize=(8, 4), subtitle=False,
                       savefig={'fname': figfile, 'format': figfmt},
                       show=False, ylabel=False, cumulative=compounded,
                       prepare_returns=False)
    ...
    figfile = _utils._file_stream()
    _plots.yearly_returns(m_returns, benchmark, grayscale=grayscale,
                          figsize=(8, 4), subtitle=False,
                          savefig={'fname': figfile, 'format': figfmt},
                          show=False, ylabel=False, compounded=compounded,
                          prepare_returns=False)
    ...
    # figfile = _utils._file_stream()
    # _plots.histogram(m_returns, grayscale=grayscale,
    #                  figsize=(8, 4), subtitle=False,
    #                  savefig={'fname': figfile, 'format': figfmt},
    #                  show=False, ylabel=False, compounded=compounded,
    #                  prepare_returns=False)
    ...
    # figfile = _utils._file_stream()
    # _plots.daily_returns(m_returns, grayscale=grayscale,
    #                      figsize=(8, 3), subtitle=False,
    #                      savefig={'fname': figfile, 'format': figfmt},
    #                      show=False, ylabel=False,
    #                      prepare_returns=False)
    ...
    # if benchmark is not None:
    #     figfile = _utils._file_stream()
    #     _plots.rolling_beta(m_returns, benchmark, grayscale=grayscale,
    #                         figsize=(8, 3), subtitle=False,
    #                         window1=win_half_year, window2=win_year,
    #                         savefig={'fname': figfile, 'format': figfmt},
    #                         show=False, ylabel=False,
    ...
    # figfile = _utils._file_stream()
    # _plots.rolling_volatility(m_returns, benchmark, grayscale=grayscale,
    #                           figsize=(8, 3), subtitle=False,
    #                           savefig={'fname': figfile, 'format': figfmt},
    #                           show=False, ylabel=False, period=win_half_year,
    #                           periods_per_year=win_year)
    ...
    # figfile = _utils._file_stream()
    # _plots.rolling_sharpe(m_returns, grayscale=grayscale,
    #                       figsize=(8, 3), subtitle=False,
    #                       savefig={'fname': figfile, 'format': figfmt},
    #                       show=False, ylabel=False, period=win_half_year,
    #                       periods_per_year=win_year)
    ...
    # figfile = _utils._file_stream()
    # _plots.rolling_sortino(m_returns, grayscale=grayscale,
    #                        figsize=(8, 3), subtitle=False,
    #                        savefig={'fname': figfile, 'format': figfmt},
    #                        show=False, ylabel=False, period=win_half_year,
    #                        periods_per_year=win_year)
    ...
    # figfile = _utils._file_stream()
    # _plots.drawdowns_periods(m_returns, grayscale=grayscale,
    #                          figsize=(8, 4), subtitle=False,
    #                          savefig={'fname': figfile, 'format': figfmt},
    #                          show=False, ylabel=False, compounded=compounded,
    #                          prepare_returns=False)
    ...
    # figfile = _utils._file_stream()
    # _plots.drawdown(m_returns, grayscale=grayscale,
    #                 figsize=(8, 3), subtitle=False,
    #                 savefig={'fname': figfile, 'format': figfmt},
    #                 show=False, ylabel=False)
    # tpl = tpl.replace('{{dd_plot}}', _embed_figure(figfile, figfmt))
